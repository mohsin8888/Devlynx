...
const Contact_Us = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      const whatsappMessage = `Name: ${formData.name}%0ACompany: ${
        formData.company_name || "N/A"
      }%0AEmail: ${formData.email}%0APhone: ${formData.phone}%0AMessage: ${
        formData.message
      }`;

      // WhatsApp link with form data
      const whatsappLink = `https://api.whatsapp.com/send?phone=923166618694&text=${whatsappMessage}`;
    ...
    <>
      <MainWrapper CustomClass={" !px-2 sm:!px-16 text-center sm:text-start"}>
        <Heading
          heading={"Contact"}
          CustomClasshea={
            " text-[3.5rem] sm:!text-[4rem] md:!text-[5rem] !text-[#000] !pb-4 sm:!pb-12 md:!pb-16"
          }
        />
        <div className="w-[90%] mx-auto flex flex-col md:flex-row gap-20">
          <div className="md:w-1/2 mb-8 md:mb-0">
            <h2 className=" poppins font-[400] text-[1.5rem] mb-4">
              Bringing your dream digital ideas to life with our expertise
            </h2>
            <h1 className="poppins font-[600] text-[2.4rem] mb-6">
              Devlynx Technologies
            </h1>
            <p className="text-lg mb-4">
              Office 504, 4th floor, Link Arcade, Model Town, Link Road, Lahore
            </p>
            <p className="text-[1.2rem] poppins font-[400] mb-4">
            ...
            {Socialmediaitem.map((item, index) => (
              <div key={index}>
                <ul className="flex justify-center md:justify-start space-x-4">
                  ...
                  <li>
                    <Link
                      to={item.whatsappfooterlink}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <img
                        src={item.whatsappfooter}
                        alt="WhatsApp"
                        className="w-6 h-6"
                      />
                    </Link>
